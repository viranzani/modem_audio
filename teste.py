...
def generate_tone(bit_number, duration_seconds):
    ...
    base_freq = min_freq + (bit_number / 16) * (max_freq - min_freq)  # 4 bits
    time = np.linspace(0, duration_seconds, int(sample_rate * duration_seconds))  # Creating the time vector
    frequency_array = np.linspace(base_freq, base_freq + (max_freq - min_freq), len(time))  # Creating the freq. vector
